...
export const gradeTls = (result: Pick<CheckResult, "status" | "problems" | "tlsVersion" | "cipherSuite" | "daysRemaining"> & { subjectAltNames?: string[]; tlsSupportedVersions?: string[] }, policy: TlsPolicySettings = defaultPolicy) => {
  let score = 100;
  ...
  if (/CBC/i.test(result.cipherSuite ?? "")) score -= 15;
  if (/^TLS_RSA_|_RSA_WITH_/i.test(result.cipherSuite ?? "") && !/ECDHE|DHE/i.test(result.cipherSuite ?? "")) score -= 20;
  if ((result.tlsSupportedVersions ?? []).some((version) => version === "TLSv1" || version === "TLSv1.1")) score -= 20;
  if (policy.profile === "strict" && (result.tlsSupportedVersions ?? []).length && !result.tlsSupportedVersions?.includes("TLSv1.3")) score -= 10;
  ...
  if (result.problems.some((problem) => /TLS assessment:.*critical|weak cipher|only \d+ bits|no modern TLS/i.test(problem))) score -= 25;
