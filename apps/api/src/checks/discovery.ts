...
export const discoverMonitors = async (domain: string): Promise<DiscoveredMonitor[]> => {
  ...
  try {
    ...
    for (const record of mx.slice(0, 3)) {
      suggestions.push(
        { name: `${record.exchange} SMTP STARTTLS`, host: record.exchange, port: 587, type: "smtp_starttls", tags: ["discovered", "mail", "mx", clean] },
        { name: `${record.exchange} IMAPS`, host: record.exchange, port: 993, type: "imaps", tags: ["discovered", "mail", "mx", clean] }
      );
    }
  } catch {
