...
export const sendStatusSubscriptionOptIn = async (subscription: StatusSubscription) => {
  ...
  if (subscription.type === "webhook") {
    return postJson(subscription.target, {
      ...
      message: "Confirm this sender.report status page subscription before incident updates are sent.",
  ...
  await transport.sendMail({
    from: smtp.from || "sender.report@localhost",
    to: subscription.target,
    subject: "[sender.report Status] Confirm your subscription",
    text: `Confirm your sender.report status page subscription before incident updates are sent.
...
const sendStatusSubscription = async (subscription: StatusSubscription, monitor: Monitor, result: CheckResult, event: "opened" | "resolved") => {
  ...
  await transport.sendMail({
    from: smtp.from || "sender.report@localhost",
    to: subscription.target,
    subject: `[sender.report Status] ${event === "resolved" ? "Resolved" : "Incident"}: ${monitor.name}`,
...
const sendChannel = async (channel: NotificationChannel, monitor: Monitor, result: CheckResult, recipient = "") => {
  ...
  if (channel.type === "pushover") return postForm(endpoint(channel, ["https://api", "pushover", "net/1/messages.json"]), {
    ...
    message: result.message,
    title: `[sender.report] ${monitor.name}`,
...
const sendEmail = async (channel: NotificationChannel, monitor: Monitor, result: CheckResult, recipient = "") => {
  ...
  await transport.sendMail({
    from: String(channel.config.from ?? smtp.from ?? "sender.report@localhost"),
    to: recipient || String(channel.config.to ?? channel.config.username ?? ""),
    subject: `[sender.report] ${subjectFor(result.severity)}: ${monitor.name}`,
...
sender.report URL: ${env.baseUrl}/monitors/${monitor.id}
`;
...
const sampleResult = (): CheckResult => ({
  ...
  status: "WARNING",
  severity: "warning",
  message: "sender.report test notification.",
...
const providerPayload = (channel: NotificationChannel, monitor: Monitor, result: CheckResult) => {
  ...
  if (channel.type === "teams") return { title: `sender.report ${result.status}`, text: `${monitor.name}: ${result.message}` };
  if (channel.type === "ntfy") return { topic: channel.config.topic, title: `sender.report ${result.status}`, message: `${monitor.name}: ${result.message}`, priority: result.severity === "critical" ? 5 : 3, tags: ["warning"] };
  if (channel.type === "gotify") return { title: `sender.report ${result.status}`, message: `${monitor.name}: ${result.message}`, priority: result.severity === "critical" ? 8 : 4 };
  return payload;
};
