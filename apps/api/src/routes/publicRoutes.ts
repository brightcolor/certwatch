...
publicRoutes.get("/status/:tag.html", (req, res) => {
  const status = publicStatus(req.params.tag);
  res.type("html").send(`<!doctype html><html><head><title>CertWatch ${escapeHtml(status.tag)}</title><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{font-family:system-ui;margin:32px;background:#0d1117;color:#e6edf3}.item{border:1px solid #30363d;border-radius:8px;padding:14px;margin:10px 0}.OK{color:#3fb950}.WARNING{color:#d29922}.CRITICAL,.DOWN{color:#f85149}.PAUSED,.UNKNOWN{color:#8b949e}</style></head><body><h1>CertWatch status: ${escapeHtml(status.tag)}</h1><p>${status.summary}</p>${status.monitors.map((monitor) => `<div class="item"><strong>${escapeHtml(monitor.name)}</strong> <span class="${monitor.status}">${monitor.status}</span><br><small>${escapeHtml(monitor.host)}:${monitor.port} - ${escapeHtml(monitor.message)}</small></div>`).join("")}</body></html>`);
});
...
publicRoutes.get("/badge/:id.svg", (req, res) => {
  ...
  const color = status === "OK" ? "#3fb950" : status === "WARNING" ? "#d29922" : status === "CRITICAL" || status === "DOWN" ? "#f85149" : "#8b949e";
...
publicRoutes.get("/status/:tag", (req, res) => res.json(publicStatus(req.params.tag)));

const publicStatus = (tag: string) => {
  const latest = results.latestByMonitor();
  const selected = monitors.list().filter((monitor) => monitor.tags.includes(tag));
