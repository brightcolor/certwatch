    ...
    <section className="content">
      <div className="grid two">
        <div className="panel">
          ...
          <button className="success" onClick={async () => setResult(await onImport(text))}>Import monitors</button>
        ...
        <div className="panel">
          ...
          <div className="actions"><button onClick={discover}>Discover</button>{!!discovery.length && <button className="ghost" onClick={addDiscoveryToImport}>Add to import</button>}{!!discovery.length && <button className="success" onClick={() => acceptDiscovery(discovery)}>Accept all</button>}</div>
          <div className="stack-list">{discovery.map((item) => <div key={`${item.host}-${item.port}-${item.type}`}><strong>{item.name}</strong><span>{item.host}:{item.port}</span><small>{item.type} - {item.tags.join(", ")}</small><button className="success" onClick={() => acceptDiscovery([item])}>Accept</button></div>)}</div>
        </div>
      </div>
      <div className="grid two">
        <div className="panel">
          ...
          <a className="button-link" href="/api/export/backup.json">Download backup JSON</a>
        </div>
        <div className="panel">
          <h3>Restore</h3>
          <label>Backup JSON<textarea value={backup} placeholder="{ ... }" onChange={(e) => setBackup(e.target.value)} /></label>
          <button className="warning" disabled={!backup.trim()} onClick={restore}>Restore backup</button>
        </div>
      </div>
    </section>
  );
