    ...
    <div className="modal">
      <form className="modal-panel" onSubmit={(e) => { e.preventDefault(); onSave(data()); }}>
        ...
        {collectsCertificate(form.type, form.config, Number(form.port)) && <FormSection title="TLS validation">
          ...
          {!sslLabsEligible(form.type, Number(form.port), form.config) && <p className="muted">SSL Labs is available for public HTTPS hosts on port 443. STARTTLS, mail, and private targets continue to use sender.report's local TLS checks.</p>}
        </FormSection>}
